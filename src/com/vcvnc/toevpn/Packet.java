...
public class Packet implements Serializable {
	...
	/**
	 * IP头部总共20个字节 char m_cVersionAndHeaderLen; //版本信息(前4位)，头长度(后4位) 1个 char
	 * m_cTypeOfService; // 服务类型8位 1个 short m_sTotalLenOfPacket; //数据包长度 2个 short
	 * m_sPacketID; //数据包标识 2个 short m_sSliceinfo; //分片使用 2个 char m_cTTL; //存活时间 1个
	 * char m_cTypeOfProtocol; //协议类型 1个 short m_sCheckSum; //校验和 2个 unsigned int
	 * m_uiSourIp; //源ip 4个 unsigned int m_uiDestIp; //目的ip 4个
	 */
	...
	/**
	 * short m_sSourPort; // 源端口号16bit short m_sDestPort; // 目的端口号16bit unsigned int
	 * m_uiSequNum; // 序列号32bit unsigned int m_uiAcknowledgeNum; // 确认号32bit short
	 * m_sHeaderLenAndFlag; // 前4位：TCP头长度；中6位：保留；后6位：标志位 short m_sWindowSize; //
	 * 窗口大小16bit short m_sCheckSum; // 检验和16bit short m_surgentPointer; //
	 * 紧急数据偏移量16bit
	 */
